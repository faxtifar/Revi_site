...
const socials = [
  { label: "TikTok / настоящий профиль", handle: "@revi_robloxer", href: "https://www.tiktok.com/@revi_robloxer", icon: Play },
    ...
    <main className="site-shell">
      ...
      <section className="hero" id="top">
        <div className="hero-copy">
          ...
          <div className="hero-footnote"><span className="scribble">↳</span> TikTok / @revi_robloxer · 56,7 тыс. лайков</div>
        </div>
      ...
      <section className="archive-section" id="archive">
        ...
        {activeTab === "archive" ? <div className="archive-list">{archive.map((item, index) => <article className={`archive-card tone-${item.tone}`} key={item.number}><div className="card-meta"><span>{item.number}</span><span>{index === 1 ? "LIVE CUT" : "ROBLOX LOG"}</span></div><div className="archive-image"><img src={item.image} alt={item.title} /><span className="play-icon"><Play size={17} fill="currentColor" /></span></div><h3>{item.title}</h3><p>{item.note}</p><a href="https://www.tiktok.com/@revi_robloxer" target="_blank" rel="noreferrer">смотреть у @revi_robloxer <ArrowUpRight size={14} /></a></article>)}</div> : <div className="notes-card"><span className="note-quote">“</span><p>Иногда я захожу просто на пять минут. Потом уже ночь, новый сервер и кто-то кричит: «Реви, ты опять это сделала».</p><span className="note-sign">— заметка из лобби, 2026</span></div>}
      </section>
